    ...
    async checkForColumn(tableName, columnName) {
        ...
        return new Promise((resolve, reject) => {
            this.database.transaction((tx) => {
                ...
                if (this.debugMode) { console.log('Coumn does not exits') }
                resolve(false)
            }, function () {
                if (this.debugMode) { console.log('Coumn exits') }
                resolve(true)
            });
        })
    }
    ...
    async createTable(tableName, callback) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            tableCreator.finish((data) => {
                let controller = new DataController(this.database, tableName, data[0], data[1])
                resolve(controller)
            })
...
class DatabaseController {
    ...
    async deleteTable(tableName) {
        return new Promise((resolve, reject) => {
            this.database.transaction((tx) => {
                ...
                if (this.debugMode) { console.log('Unable to delete talbe') }
                reject()
            }, function () {
                if (this.debugMode) { console.log('Table deleted') }
                resolve()
    ...
    finish(callback) { this.finishFunction = callback }
}
...
class DataController {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.database.transaction((tx) => {
                tx.executeSql(insertString, [], (tx, results) => {
                    if (this.debugMode) { console.log('INETER RESULTS | ' + "insertId: " + results.insertId + ' | ' + "rowsAffected: " + results.rowsAffected) }
                    resolve()
                });
    ...
    update(rowId, columnName, newValue) {
        return new Promise((resolve, reject) => {
            ...
            if (!rowId && rowId != 0) { return console.error('No row id value specified') }
            ...
            this.database.transaction((tx) => {
                tx.executeSql(updateString, [], (tx, res) => {
                    if (this.debugMode) { console.log('UPDATE RESULTS | ' + "updateId: " + rowId + ' | ' + "rowsAffected: " + res.rowsAffected) }
                    resolve()
                }, function (tx, error) {
