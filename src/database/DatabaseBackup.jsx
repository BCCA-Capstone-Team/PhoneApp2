      ...
      this.database.transaction(
        tx => {
          tx.executeSql(insertString, [], (tx, results) => {
            if (this.debugMode) {
              console.log(
                `INETER RESULTS | insertId: ${results.insertId} | 'rowsAffected: ${results.rowsAffected}`,
              );
            }
            resolve();
          });
      ...
      this.database.transaction(tx => {
        tx.executeSql(
          ...
          (tx, res) => {
            if (this.debugMode) {
              console.log(
                `UPDATE RESULTS | updateId: ${rowId} | rowsAffected: ${res.rowsAffected}`,
              );
            }
            resolve();
          },
      ...
      this.database.transaction(tx => {
        tx.executeSql(
          ...
          (tx, res) => {
            if (this.debugMode) {
              console.log(
                `DELETION RESULTS | deletionId: ${indexNumber} | rowsAffected: ${res.rowsAffected}`,
              );
            }
            resolve();
          },
